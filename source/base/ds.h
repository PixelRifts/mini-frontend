...
#define darray_add_all(type, to, from) type##_array##_add_all(to, from)
...
void Data##_array##_add_all(Data##_array* to, Data##_array* from);\
...
void* prev = to->elems;\
u32 new_cap = DoubleCapacity(to->cap);\
while (new_cap <= from->len) new_cap = DoubleCapacity(new_cap);\
to->elems = calloc(new_cap, sizeof(Data));\
memmove(to->elems, prev, to->len * sizeof(Data));\
to->cap = new_cap;\
free(prev);\
}\
memmove(&to->elems[to->len], from->elems, sizeof(Data) * from->len);\
to->len += from->len;\
}\
void Data##_array##_reserve(Data##_array* array, u32 count) {\
void* prev = array->elems;\
...
#define stable_table_get_guarantee(key_t, value_t, table, key, val) key_t##_##value_t##_stable_table_get_guarantee(table, key, val)
...
/* TODO I think I had forgotten this, check if it is correct */\
pool_dealloc(&table->element_pool, curr);\
} else {\
if (curr->hash_next) {\
